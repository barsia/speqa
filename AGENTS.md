...
- **Stale closure prevention in Compose effects:** Any callback parameter or external value captured inside `LaunchedEffect`, `snapshotFlow.collect`, `DisposableEffect`, `SideEffect`, or **`Modifier.pointerInput(key) { ... }`** (incl. `detectTapGestures`) with a stable key (e.g., `remember`-ed object, `Unit`, or an unchanging flag like `enabled`) MUST be wrapped with `rememberUpdatedState`. Without it, the closure captures the value from first composition and never updates, causing data loss/reverts (e.g. a click handler that keeps seeing the original `testCase`, so every subsequent add overwrites the previous one). Pattern: `val currentCallback by rememberUpdatedState(callback)` then use `currentCallback` inside the effect/pointerInput block. This applies to `onValueChange`, `onTestCaseChange`, `onFocusConsumed`, any `onClick`/`onTap` routed through `pointerInput`, and any lambda parameter used in long-lived coroutines. Stale closures in `forEachIndexed` loops have the same issue — use `rememberUpdatedState` for the parent state and read `.value` inside lambdas.
- **Delete-focus restoration in list UIs:** For any composable that renders a list of deletable items plus an add-button (ticket chips, link rows, attachment rows, and future similar surfaces), use `rememberDeleteFocusRestorer` from [`editor/ui/DeleteFocusRestorer.kt`](src/main/kotlin/io/github/barsia/speqa/editor/ui/DeleteFocusRestorer.kt) — don't roll per-site `pendingRestore`/`LaunchedEffect` pairs. Attach `restorer.itemRequesters[index]` to each row and call `restorer.onDeleted(index, sizeBefore)` in the delete handler. It moves focus to the neighbor (same index if not last, previous if last) or to the add-button when emptying the list, matching the A11y contract the rest of the editor already follows. The pure decision lives in `nextFocusTargetAfterDelete` and is covered by `DeleteFocusTargetTest`.
