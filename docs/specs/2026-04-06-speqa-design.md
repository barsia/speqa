...
- **Visual markdown fidelity:** Read-only text in SpeQA visual editors must reflect inline Markdown authoring from the left editor instead of exposing raw markup. At minimum, strong emphasis, emphasis, inline code, and inline links (`[text](http(s)://...)`) are rendered visually in description/preconditions blocks and in step action/expected text for both test-case preview and test-run preview. An inline link shows only its link text, styled with the editor/Markdown link foreground and underlined, while the `[`, `]`, and `(url)` syntax is folded away. The visual editor does not need to expose full Markdown authoring UI, but it must not silently drop formatting or show raw `**...**` or `[text](url)` markup in read-only text.
...
- **Post-creation reveal and focus:** After a test run is written (single-case via `startTestRun`, multi-case via `CreateTestRunAction` / `CreateMultiCaseRunWriter`), the run is immediately usable: the new `.tr.md` opens in its split editor with keyboard focus on the editor, the SpeQA tool window's TRs tab is activated and selected, and the new run's row is selected in that tab's tree. Activating the tool window must not steal focus back from the run editor, so the caret stays in the run for setting verdicts right away. Both creation flows share one helper (`openAndRevealTestRun`); the reveal is delivered through `SpeqaTestRunRevealService`, a project service the tool window registers a revealer with on each content build, so the creation flow does not depend on the tool-window UI being open. The per-node tree-walk decision (select the target row, descend into an ancestor directory, or skip) is pure (`revealStepFor`) and unit-tested.
- **Run import-options UX:** `RunCreationDialog` includes six independent import checkboxes: `Description`, `Tags`, `Environment`, `Tickets`, `Links`, and `Attachments`. Defaults: all six options are disabled (unchecked) - a run starts as a clean result sheet and the user opts into copying source content, consistent with the multi-case dialog. `Description` controls whether the source case body blocks (description and preconditions) are copied into the run. If the source test case has no data for a category, the corresponding checkbox is disabled, forced to the unchecked state, and exposes a hover tooltip explaining that there is nothing to import. The dialog request payload carries these choices as `RunImportOptions`, and `startTestRun` computes the per-category availability from the parsed source `TestCase` before opening the dialog. Availability must match what `TestRunSupport.createInitialRun(...)` can actually import in the current product slice: for `Links`, that means top-level test-case links only, not step-level links.
- **Run import-options layout:** the six import checkboxes in `RunCreationDialog` are shown in a compact two-column grid instead of one long vertical stack. The section label stays above the grid, and the checkboxes retain their order: `Description`, `Tags`, `Environment`, `Tickets`, `Links`, `Attachments`.
...
- **Keyboard focus ring (custom chips, rows, and icon buttons):** The focusable custom preview controls (tag/environment chips `TagChip`, ticket chips `TicketChip`, link rows `LinkRow`, attachment rows `AttachmentRow`, and the shared icon-only action buttons from `speqaIconButton` such as add, edit pencil, and remove `X`) render their keyboard-focus indicator as a thin (1 px, theme-scaled) rounded outline in the IDE theme focus color (`JBUI.CurrentTheme.Focus.focusColor()`), not the heavy platform glow border. The ring, and the progressive-disclosure remove `X`, appear only when the control receives focus via keyboard traversal (`FocusEvent.Cause.TRAVERSAL*`); mouse-click focus and programmatic focus (for example the focus restored onto a neighbour after a sibling is deleted via its remove `X`) must not paint the ring or reveal the remove affordance. Native widgets such as the Priority/Status/Result `ComboBox` dropdowns and text fields intentionally keep the standard IntelliJ focus ring rather than this compact one. The cause gating and the ring painter are shared in `editor/ui/primitives/FocusRing.kt` (`isKeyboardFocusCause`, `paintCompactFocusRing`) so these controls stay consistent.
- **Chip/row sub-action keyboard model:** Tag/environment chips and link/attachment/ticket rows (both at the top level and under steps) are each a single keyboard Tab stop; their sub-action glyphs (the edit pencil and the remove `X`) are NOT Tab stops. They are kept out of the Tab chain and the accessibility focus tree by being non-focusable (mouse-only affordances). On a focused chip/row: `Delete`/`Backspace` removes the value; `F2` edits it where editing applies; `Enter` (and `Space`) performs the element's primary action, identical to a left-click (filter by the value for chips, open the target for link/attachment/ticket rows). The pencil and `X` stay mouse affordances, revealed on hover and on chip/row keyboard focus so the user can see what `F2`/`Delete` will act on; the focus ring is painted on the chip/row itself, never on the glyphs. Each control names its keys for discoverability: the chip/row tooltip and the revealed-glyph tooltips include the key (for example `Edit (F2)`, `Remove (Delete)`), all via `SpeqaBundle`, and the control's `AccessibleContext` name is the value with a description carrying the full verb set (for example `Tag. Enter to filter, F2 to edit, Delete to remove`) so a screen reader announces the actions even though the glyphs are not focusable. Each group is exposed as a list (Tags, Environment, Links, Attachments, Tickets) whose items are the chips/rows; the trailing add (`+`) button is its own always-visible Tab stop. Down-to-enter sub-action navigation is intentionally not used; optional `Left`/`Right` roving between sibling chips in a group is permitted as additive sugar.
- **Dialog and chooser focus restoration:** Opening a modal dialog or native chooser from the preview (the Add/Edit Link dialog and the attachment file chooser) captures the triggering control (or the current focus owner) and restores focus to it via `requestFocusInWindow()` after the dialog closes on any outcome (OK, Cancel, or `Escape`), so keyboard focus is never dropped out of the editor. This mirrors the tag/environment add-popup anchor-restore rule.
- **Delete focus restoration timing:** Focus restoration after removing a list item (`DeleteFocusRestorer`) requires the owning section to rebuild synchronously before `onDeleted` runs, so focus lands on the surviving neighbour component (the new occupant of the deleted slot, the previous item, or the add button) rather than on a stale component that the asynchronous document round-trip then discards. Section setters (`setTags`/`setLinks`/`setAttachments` and the like) are idempotent so the trailing asynchronous refresh does not rebuild again and re-drop focus. Top-level sections that hide their own inline add button pass the external section-header `+` as the restorer's add-button target.
...
- `TestRunSupport.createInitialRun` accepts explicit import options from `RunCreationDialog`. It always copies step `action` / `expected`. It conditionally copies body blocks (the case description / preconditions, gated by the `Description` option), `tags`, `environment`, top-level `links`, top-level `attachments`, and step `tickets` / `links` / `attachments` according to the chosen checkboxes. No imported category may be silently dropped during `TestCase -> TestRun` creation.
...
- `TestRun` has fields: `id`, `title`, `tags`, `priority: Priority? = null`, `manualResult`, `startedAt`, `finishedAt`, `result`, `environment`, `runner`, `bodyBlocks: List<TestCaseBodyBlock> = emptyList()`, `links: List<Link> = emptyList()`, `attachments: List<Attachment> = emptyList()`, `comment: String = ""`, `stepResults`. `priority` is copied from the test case at run creation. Step `action` / `expected` are always copied from the test case as the initial run snapshot, and remain editable afterwards inside the run. `bodyBlocks`, `tags`, `environment`, top-level `links`, top-level `attachments`, and step `tickets` / `links` / `attachments` are imported according to the user's run-creation options (`bodyBlocks` gated by the `Description` option) and remain editable afterwards. The `comment` field stores an overall run comment (text after the last step in the file body).
...
- `TestRunSupport.createInitialRun` accepts explicit import options from `RunCreationDialog`. It always copies `priority` and step `action` / `expected`. It conditionally copies `bodyBlocks` (gated by the `Description` option), `tags`, `environment`, top-level `links`, top-level `attachments`, and step `tickets` / `links` / `attachments` based on the chosen checkboxes. Sets `startedAt = null` (not started yet). No longer passes `startedAt`, `finishedAt`, or `result` explicitly — they use defaults.
...
- Test run files carry a self-contained snapshot of the test-case scenario. Step `action` / `expected` are always present. `bodyBlocks`, `tags`, `environment`, top-level `links`, top-level `attachments`, and step `tickets` / `links` / `attachments` are present only if imported at creation time or later added by the user while editing the run.
...
- **List auto-continuation in multi-line fields:** All multi-line preview text fields (Action, Expected result, Description, Preconditions, and run comments) support automatic list continuation. When the user presses Enter on a line starting with a markdown list marker (`- `, `* `, or `N. `), the next line automatically receives the continuation marker (same bullet or incremented number). Pressing Enter on an empty marker (e.g. `- ` or `3. ` with no content after) removes the marker and exits list mode. The implementation is owned by `MarkdownEditablePane`: a single `installEnterHandling` action is bound to plain Enter (no modifiers) on the embedded editor's content component. One action handles both cases (it is NOT two separate ENTER bindings for list-continuation and code-block, which produced an ambiguous shortcut that silently swallowed Enter in the Description / Preconditions panes): inside an indented code block it inserts a newline, otherwise it delegates to `MarkdownEnterHandler.apply`, which runs the pure `ListContinuation.onEnter(text, caret)` and applies the replacement through a `WriteCommandAction` plus a caret move. The command wrapper matters - a bare `runWriteAction { document.replaceString(...) }` does not reliably commit a change to the EditorEx-backed document, so the earlier version did nothing on Enter inside a Preconditions list. The IntelliJ Markdown plugin's own Enter handler does not reach the light virtual document backing `EditorTextField`, so the behaviour must be installed explicitly. The same pure `ListContinuation` helper is also used by the legacy `MarkdownPane` (read-mostly prose pane) so both surfaces stay in lockstep; `MarkdownEnterHandlerTest` covers the apply path on a real editor.
- **List item indentation with Tab in the `.tc.md` / `.tr.md` source editor:** In the native Markdown source editor for Speqa files, pressing `Tab` (or `Shift+Tab`) while the caret is on a markdown list item line (bullet `- `/`* ` or ordered `N. `) indents (or outdents) that whole list item: its marker line plus every continuation line that belongs to it, including expected-result blockquote (`>`) lines and wrapped continuation text. Indentation is always added to the left of each line (its leading whitespace), never inside a continuation block; specifically, the `>` blockquote marker keeps its position relative to its own line and spaces are never inserted after `>`. This replaces the bundled IntelliJ Markdown plugin's own Tab/indent action, which reindents a list item's child blocks by inserting spaces after the blockquote marker (producing `>    text`) and so corrupts the shared scenario step shape. The indent unit equals the current item's marker width (the `N. ` / `- ` prefix length excluding its own leading indent), so one Tab nests the item exactly one level under its list. A continuation line belongs to the item when its leading-whitespace width is at least the item's content column (`leadingIndent + markerWidth`); a blank line or a less-indented line terminates the item. `Shift+Tab` removes up to one indent unit of leading whitespace from each of those lines. The decision is a pure helper, `ListIndent.onTab(text, cursor)` / `onShiftTab(text, cursor)` (returns the new document text plus caret, or `null` to fall through). It is applied by `SpeqaTabActionHandler`, an `EditorActionHandler` that wraps the platform `EditorTab` and `EditorUnindentSelection` handlers and is installed at startup by `SpeqaTabActionSetup` (mirroring `SpeqaBackspaceActionHandler` / `SpeqaBackspaceActionSetup`). The handler only acts on Speqa files with a single caret and no selection; otherwise, and whenever the helper returns `null` (non-list line, code block, plain paragraph), it defers to the wrapped platform handler so default Tab behavior is preserved. This applies to the raw source editor only; in the SpeQA preview, `Tab` remains field-to-field navigation.
...
- **Run title editing:** the test-run title uses the same inline-editing mechanics as the test-case title. `InlineEditableTitleRow` is reused: pencil to enter edit mode, `Enter` to commit, `Escape` to cancel, blur to commit, and focus return to the title affordance after save/cancel. While editing, the pencil shows a save (disk) icon and `Save` tooltip; clicking it commits. The title commits on Enter / focus loss / the disk button, not live per keystroke (a live per-keystroke commit was tried and reverted: the per-keystroke document rewrite destabilized the field's focus and caret).
...
A dedicated **SpeQA** tool window provides a curated, test-centric view of the project, separate from the platform Project view. It answers "what test cases and test runs exist and how are they organized" without the noise of source files or configs.

**Tabs:** the tool window header carries two content tabs next to the window name, switched via the platform content manager:
- **TCs** - the test-cases tree, rooted at `test-cases/`.
- **TRs** - the test-runs tree, rooted at `test-runs/`.

Both tabs are always present (the TRs tab is shown even when no runs exist yet). The two tabs are structurally identical (folder sections, leaves labeled by `title`, natural ordering, live VFS updates, persisted expansion state) and differ only in their root directory, the leaf file type (`.tc.md` vs `.tr.md`), the leaf icon (status stamp vs run-result stamp), and the primary filter facet (Status vs Result). Each tab owns an independent in-memory filter; switching tabs does not carry filter state across. Filter title-bar actions are window-global, so the active tab's filter actions are installed (`ToolWindow.setTitleActions`) and swapped whenever the selected content changes. The shared tree machinery is parameterized by a `SpeqaLeafSpec` strategy (is-leaf, is-filtering, matches, title, icon); a single `SpeqaFolderNode` + `SpeqaLeafNode` pair drives both tabs.

**Placement & availability:**
- Anchored on the **left** stripe (`anchor="left"`, `secondary="false"`). A `defaultToolWindowLayout` extension (`SpeqaToolWindowLayout`) places SpeQA **first** on the left stripe in the default layout (order 0). First is the only stable position: conditional tool windows like Commit register after startup and reclaim their slot, so any non-first placement gets reshuffled (a second-place SpeQA is pushed to third once Commit appears); nothing can reclaim a slot ahead of order 0. The default layout is built from a single shared insertion-ordered map and `addOrUpdate` keeps an existing id at its position (no insert-at-index), so the extension lifts every left window out of the layout, adds SpeQA, then re-adds the lifted windows after it, preserving each window's settings via the public builder (visibility, weight, content UI type, split, side weight). This affects only the default layout (first run and Restore Default Layout); once the user rearranges the stripe, their saved layout takes over. Existing projects with a saved layout that predates SpeQA keep that layout (the platform-correct behavior) and can pick up the new default via Restore Default Layout. The tool window is **not** auto-opened: in the new UI the default-layout `isVisible` does not expand a window (that needs `isActiveOnStart`, absent from the public builder API), and opening it at runtime would show another window first and visibly switch, so the user opens SpeQA themselves. There is deliberately no runtime stripe-reordering, no auto-activation, and no reflection.
...
- The tool window is always available (`ToolWindowFactory.shouldBeAvailable` returns `true`), so its stripe button is present in every project. When the project has neither a `test-cases/` nor a `test-runs/` directory, both tabs are still shown with their empty-state panels (see "Empty tool window" below), and the content is rebuilt when the first SpeQA directory appears. When only one of the two directories exists, both tabs are still shown; the tab whose root directory is missing renders an empty tree with its empty-state text.

**Create actions (per tab):** each tab's title bar carries a leading "+" action (`AllIcons.General.Add`), placed before the four filter facets and separated from them by a divider:
- The **TCs** "+" creates a new test case. It is always enabled; it prompts for the new test case name, creates the `test-cases/` directory first if it does not exist, generates the next free test-case id, instantiates the `SpeQA Test Case.tc.md` template, and opens the new file in the editor.
- The **TRs** "+" opens the Create Test Run dialog (see section 15g). It is enabled only when at least one test case exists, and is disabled with an explanatory tooltip otherwise.

Both "+" actions follow the **same placement rule**, derived from the active tab's tree selection at the moment the action runs:
- a selected **leaf** (test case or test run) -> the new file is created as a **sibling** in that leaf's folder;
- a selected **folder** -> the new file is created **inside** that folder;
- **nothing selected** -> the new file is created in the tab's **root** directory (`test-cases/` for a new test case, `test-runs/` for a new run).

The empty-state Create Test Run button (below) is the same TRs "+" action surfaced as a button; when invoked from the empty state there is no tree selection, so the run is created in the `test-runs/` root.

- **Tree context menu:** Right-clicking a node in either tab opens a native context menu (installed with `PopupHandler`; the tree publishes `CommonDataKeys.VIRTUAL_FILE`/`PSI_ELEMENT`/`NAVIGATABLE` and a delete provider through `UiDataProvider`, so platform actions work natively). Items are node-kind aware:
  - **Test case `.tc.md` leaf (TCs tab):** Open, Run Test Case (single-case run via `Speqa.RunTestCase`), then Rename, Delete, Reveal in Project View, Reveal in Finder.
  - **Test run `.tr.md` leaf (TRs tab):** Open, then Rename, Delete, Reveal in Project View, Reveal in Finder.
  - **Folder (TCs tab):** New Test Case (created inside the folder), Create Test Run, then Rename, Delete, Reveal in Project View, Reveal in Finder. The folder's Create Test Run opens the Create Test Run dialog scoped to that folder's subtree: only test cases under the folder are offered as candidates and they are all pre-selected. It is disabled when the folder's subtree contains no test cases.
  - **Folder (TRs tab):** Create Test Run (the new run is written into that folder; candidates are all project test cases, all pre-selected), then Rename, Delete, Reveal in Project View, Reveal in Finder.
  - Rename, Delete, Reveal in Project View, and Reveal in Finder are the platform actions (`RenameElement`, `$Delete`, `SelectInProjectView`, `RevealGroup`); Delete operates through the platform delete provider with its standard confirmation.
- **Create Test Run default selection:** the Create Test Run dialog pre-checks every candidate case it shows, in all entry points (TRs "+", empty-state button, and the tree context menu). The user narrows the set with the filters and the Clear/Select all buttons.
- **Empty state (TCs):** when the TCs tab has no test cases, it shows the same style of centered empty-state panel as the TRs tab (swapped in via a card layout that returns to the tree as soon as the first test case exists). The panel shows a large test-case stamp icon, a "No test cases yet" title, a one-line description, and a Create Test Case button. The button is always enabled; it creates a test case in the `test-cases/` root, creating that directory if it does not exist.
- **Empty tool window (no SpeQA directories):** when the project has neither a `test-cases/` nor a `test-runs/` directory, the tool window still shows both tabs with their empty-state panels (previously it added no content). Creating the first test case or test run from an empty-state button rebuilds the tool-window content so the new file appears in its tree. This supersedes the earlier "adds no content" behavior.

**Empty state (TRs):** when the TRs tab has no test runs yet, it shows an informative empty-state panel instead of the bare tree, swapped in via a card layout that returns to the tree as soon as the first run exists. The panel is centered and shows a large run-result stamp icon, a "No test runs yet" title, a one-line description of what a test run captures, and a Create Test Run button. The button is enabled only when at least one test case exists; otherwise it is disabled with a tooltip explaining that test cases must be created first, and its enabled state tracks test-case creation/removal live. The TCs tab uses the same style of panel (see "Empty state (TCs)" above) with a Create Test Case button.

**Options menu:** the tool window's options ("gear"/more-actions) menu carries an *About SpeQA* entry as its last item, mirroring how the same `Speqa.About` action sits last in the editor's entry-point menu. It is the registered `Speqa.About` action installed via `ToolWindow.setAdditionalGearActions`, and opens the standard About SpeQA dialog (plugin version and links). This is the only tool-window options-menu entry SpeQA adds.

**Tree contents - the contract (per tab):**
- **Root** is the tab's directory (`test-cases/` for TCs, `test-runs/` for TRs); the root node itself is not shown - its children are the top level of the tree.
- **Folders are sections.** Every subdirectory under the root is shown as a folder node, **including empty folders and folders that (transitively) contain no leaves** (when no filter is active; see "Filters" below). The folder hierarchy mirrors the on-disk directory structure exactly.
- **Leaves are labeled by `title`.** Each leaf file (`.tc.md` in TCs, `.tr.md` in TRs) is a leaf node whose displayed text is the `title` parsed from YAML frontmatter (not the file name). A test case with a blank/absent title falls back to `"Untitled Test Case"`; a test run falls back to `"Untitled Test Run"`. The TCs leaf shows the test-case stamp icon (matching its status); the TRs leaf shows the run-result stamp icon (`SpeqaIcons.forResult`).
- **Other files are hidden.** A tab shows only its own leaf type; regular files (`README.md`, attachment images, the other tab's file type, etc.) never appear.
- **No horizontal scrollbar.** The tree never grows wider than its viewport: each tab's `Tree` reports `getScrollableTracksViewportWidth() = true`, has horizontal auto-scrolling disabled, and is wrapped in a scroll pane whose horizontal scrollbar policy is `HORIZONTAL_SCROLLBAR_NEVER`. A leaf or folder label too long for the current width is clipped and revealed in full by the platform's expandable-item hover popup (the same affordance native IntelliJ trees use), not by a horizontal scrollbar.

**Ordering within a node:** folders first (by name), then leaves ordered by `title`, both case-insensitive and natural-sort (so `Step 2` precedes `Step 10`). This ordering plus the leaf-only / folders-first filtering is a pure function covered by unit tests.

**Interaction:** double-click or Enter on a leaf opens the file via `FileEditorManager` (which routes to the matching SpeQA split editor). Folder nodes expand/collapse. Standard platform speed-search (type-to-filter) is available because the tree is built on the platform tree framework.

**Filters:** a header above the tree provides a multi-facet filter across four facets. The first facet is tab-specific (**status** in TCs, **result** in TRs); the other three (**priority**, **tags**, **environment**) are shared.

Facets and their values:
- **Status** (TCs) - single-select enum: `All statuses` (default) plus Draft / Ready / Deprecated, each rendered with its stamp icon (`All statuses` has no icon).
- **Result** (TRs) - single-select enum: `All results` (default) plus Passed / Failed / Blocked / In Progress / Not Started; Passed/Failed/Blocked carry their run-result stamp icon, the rest have none.
- **Priority** - single-select enum: `All priorities` (default) plus Critical / Major / Normal / Low.
- **Tags** - multi-select over the tab's known tags (TCs from `SpeqaTagRegistry.allTags`, TRs from `SpeqaTagRegistry.allTestRunTags`), picked via the existing tag autocomplete/chip UI.
- **Environment** - multi-select over the tab's known environments (TCs `allEnvironments`, TRs `allTestRunEnvironments`), same picker UI.

Matching semantics (pure functions `matchesFilter(summary, filter)` for cases and `matchesRunFilter(summary, filter)` for runs, both unit-tested):
- A facet with no selection does not constrain results (the primary facet/priority `null`, or an empty tag/environment set).
- The primary facet (status or result) and priority match when the leaf's value equals the selection. A run with no priority is excluded when the priority facet is set.
- Tags match when the leaf has **at least one** of the selected tags (OR within the facet); environment matches the same way.
- Active facets are combined with **AND** - a leaf is shown only if it satisfies every active facet.

Header UI:
- The four facet triggers - the primary facet (Status or Result) plus Priority, Tags, and Environment - live in the tool-window **title bar** (next to the tool-window name/gear), installed via `ToolWindow.setTitleActions`. Each is an `AnAction` carrying the facet's icon and a tooltip naming its facet. The primary and Priority/Tags/Environment icons are project-owned 16x16 SVGs under `/icons/` (`filterStatus`, `filterPriority`, `filterTags`, `filterEnvironment`, each with a `_dark` variant), exposed through `SpeqaIcons`; the Result facet reuses the `filterStatus` funnel icon (it is the primary facet on its own tab and never shown alongside Status). Because title actions are window-global, the factory installs the currently-selected tab's title actions and swaps them whenever the content selection changes. Clicking a title-bar button opens a small `JBPopup` scoped to that one facet, anchored underneath the clicked button:
  - The primary (Status/Result) and Priority popups list `All <facet>` plus the facet's values (status and run-result values carry their stamp icon); picking a value sets that facet and closes the popup.
  - Tags and Environment popups reuse the editor's `TagCloud` + `AddTagPopup` autocomplete picker (fed by `SpeqaTagRegistry`, scoped to the active tab's known values), letting the user add/remove several values; the popup stays open while picking.
  - A facet that is active renders in the native selected/highlighted state (via `Toggleable`) so the active facets are visible at a glance.
- The four facet triggers are hidden entirely when the active tab has no leaves at all (no test cases / no test runs, ignoring any filter): there is nothing to filter, so the controls do not appear until at least one leaf exists. This is recomputed on every relevant VFS change (and nudges the title toolbar so the controls appear/disappear live), and it covers the missing-directory case (a tab whose root directory does not exist shows no filter controls).
- A "clear all" title-bar action appears only when at least one facet is active and resets every facet.
- Just above the tree, a row of removable chips shows each active selection (primary-facet value, priority value, each tag, each environment); each chip carries an always-visible close button that clears that one selection. The chip row is part of the tab's own panel, so switching tabs shows that tab's chips. The chip row is hidden when no filter is active.
...
**State persistence:** each tab's tree expansion state and selection are persisted per project (in the workspace file) and restored when the project is reopened and across IDE restarts, using the platform `TreeState` mechanism (the same one the Project view uses) held by project-level `PersistentStateComponent`s (one `@State` slot for the TCs tree, a separate one for the TRs tree). The current state is captured eagerly on every expand/collapse and selection change (and once more when the content is disposed, which includes project close), and re-applied when the content is built. Capturing eagerly rather than only on dispose is required because on project close the platform may serialize the component before the tool-window's disposable runs. Nodes are identified by their backing file, so expansion/selection survive the tree rebuilds caused by VFS changes and filtering. The in-memory filter selection itself is not persisted (see "Filters").

In addition to per-tree state, the **currently selected tab** (TCs or TRs) is persisted per project in the workspace file (its own `@State` slot) and restored when the project is reopened or the IDE restarts, so the tool window reopens on the tab the user last viewed. It defaults to the TCs tab when nothing has been stored yet.

**Live updates:** each tab's tree reflects file-system changes under its root directory without manual refresh. Creating, deleting, renaming, or moving leaf files and folders updates the tree; editing a leaf's `title` updates that leaf's label. A leaf's label and icon also reflect the live in-memory document, not only the saved file: changing a test case's status or a test run's result in the editor repaints that leaf's stamp icon (and any title/filter-relevant change repaints its label) immediately, before the file is saved to disk. Parsed summaries are cached per file and invalidated on content change so the tree does not re-read disk on every repaint. When a file has an open in-memory document, the summary is parsed from that document's text and the cache entry is keyed on the document's modification stamp; otherwise it is parsed from disk and keyed on the file's modification stamp.

**Implementation:** built on the IntelliJ platform tree framework (`AbstractTreeStructure` plus `StructureTreeModel` plus `Tree`) - the same machinery as the Project view - for native theming, async loading, and speed-search. Nodes live under a dedicated tool-window package. The factory subscribes to VFS changes scoped to each tab's root directory via the tool window's `Disposable`, and additionally to in-memory document changes (via the `EditorFactory` event multicaster) so leaf edits that are not yet saved still refresh the affected leaf.
...
- Failed — red cross
- Blocked — orange stop
- In progress - blue icon with a half-filled circle glyph (`testRunInProgress.svg`)
- Not started - neutral grey icon with no glyph (`testRunNotStarted.svg`)

All five run-result icons are custom SVGs in the shared test-run visual family (`SpeqaIcons.TestRun*`); no platform `AllIcons` fallback is used.

A freshly created run is `not_started`, so the Project View icon must give it a distinct neutral icon, never the Blocked stamp. The `SpeqaIconProvider` reads the run result from the `result:` frontmatter line (the value may contain underscores, e.g. `not_started` / `in_progress`) and delegates to `SpeqaIcons.forResult`, the same mapping the TRs tool-window leaf uses, so the two surfaces stay consistent.
...
## 15f. Multi-case run sections and section reorder

A `.tr.md` run that contains two or more cases renders through `RunMultiCasePanel` instead of the flat `TestCasePanel(mode = RUN)`. The layout is chosen once from the initial parse via `RunCasesContainer.layoutFor(run)` (`SECTIONED` when `run.cases.size >= 2`, else `FLAT`). The panel shows a run header laid out exactly like the single-case run header - a `Result | Progress` row, then a `Runner | (empty)` row (a multi-case run has no run-level priority where the single-case run shows Priority), then a right-aligned action row - above a `RunCasesContainer`: a vertical `BoxLayout.Y_AXIS` list of one collapsible `RunCaseSection` per case, separated by a scaled 16 px vertical strut. Every edit folds the changed case back into the whole `TestRun` and emits it through `onRunChange` (full reserialize; there is no per-case patch path).

**Lazy, collapsed-by-default sections.** Each `RunCaseSection` starts collapsed and builds its body (`RunCaseBody`, which is the expensive part - it instantiates priority/tag/link/attachment widgets and several `EditorFactory`-backed Markdown editors per case and per step) only the first time the section is expanded. This keeps opening and switching to a multi-case run fast regardless of case count; the header (id/title, result pill, drag handle) is always built. Collapse/expand state and any built body persist across tab switches (the sections are not rebuilt on re-show). `update(case)` refreshes a section's header always and its body only when it has been built; an expand triggered later builds the body from the latest case state.

**Overall run result.** The run-header result is a `RunResult` dropdown - the same control as the single-case run, not a read-only label. It shows `effectiveRunResult(run)` (the computed aggregate of the per-case results by default). Picking a real result pins a manual run-level override (`TestRunSupport.overrideRunResult`, which sets `TestRun.result` and `manualResult = true`) that holds regardless of the individual case results and does NOT modify any per-case result or pill; picking `Not started` clears the override (`TestRunSupport.clearRunOverride`) back to the computed aggregate. While not overridden, the stored `TestRun.result` is kept in sync with the aggregate as cases change (`TestRunSupport.syncAggregateResult`). While the run result is manually overridden, a compact icon-only manual indicator (the shared `manualResultIndicator`, hover tooltip, no text label) appears next to the dropdown. The override persists across reserialize via the run-level `result:` / `manual_result:` front-matter fields. The single-case run uses the same dropdown plus the same icon-only manual indicator for consistency.

**Per-case result override.** Each section header carries an interactive result pill (the shared `ResultPill` component) showing the case's `RunResult`. The pill is a clickable control (hand cursor, native tooltip) that opens an action-group popup listing the meaningful run results (Passed / Failed / Blocked / In progress / Not started) plus an "Auto (from steps)" entry. Picking a result calls `TestRunSupport.overrideCaseResult(case, picked)` (sets the result and `manualResult = true`); picking "Auto (from steps)" calls `TestRunSupport.clearCaseOverride(case)` (re-derives the result from the step verdicts and clears the manual flag). The chosen case is emitted through `onCaseChange`, which the container folds back into the whole run. When a case result is manually overridden (`RunCaseSectionState.isManual`), a compact icon-only manual indicator (the shared `manualResultIndicator`, hover tooltip, no text label) appears next to the pill; it is hidden when the result is derived from steps. `update(newCase)` refreshes the pill result and the manual-indicator visibility. The per-case pill, the per-case manual indicator, the overall run dropdown, and the single-case run all reuse the same `ResultPill` / `manualResultIndicator` primitives.

**Run header field order and actions.** Both the single-case and multi-case run headers order the operational fields as `Result | Progress` then `Runner | Priority`, where the multi-case run leaves the Priority slot empty (no run-level priority across multiple cases). The Progress value uses the default label foreground in both (no muted tint). A **Reset results** icon button sits next to the result control in both runs: after a confirmation dialog it clears every result via `TestRunSupport.resetResults` (all step verdicts to NONE, every case result and the run result to NOT_STARTED, manual overrides dropped, start/finish timestamps cleared; step text, comments, and metadata preserved). It is disabled (greyed, with an explanatory tooltip) when the run is already pristine (`TestRunSupport.isPristine`). The multi-case run hosts **Expand all** / **Collapse all** icon buttons in the otherwise-empty Priority slot; they call `RunCasesContainer.setAllExpanded(true/false)`. Each is disabled with an explanatory tooltip when it would be a no-op (Expand all disabled when every section is already expanded, Collapse all when every section is already collapsed); the enabled state tracks both the bulk actions and individual section collapse toggles.

**Section header layout.** The section header is a `BorderLayout`: a square collapse chevron at the far left (WEST), the `TC-<id> <title>` label in the middle (CENTER), and the result-pill cluster pinned to the right (EAST: optional manual indicator, the result pill, then the drag handle). The result pill is therefore always visible at the right edge and never pushed off-screen by a long title. The title label takes the remaining middle width and is truncated with an ellipsis when it does not fit, so the header stays a single row at the tab width (no horizontal scrollbar). The chevron uses the standard 22x22 square icon-button slot, placed before the title.

**Section drag-reorder.** Whole case sections can be reordered by dragging a section's drag handle (`RunCaseSection.dragHandle`, an `AllIcons.General.Drag` grip with the reorder tooltip and a hand cursor). The drag-and-drop machinery is reused verbatim from the step editor: `RunCasesContainer` wires `DragReorderSupport` (ghost overlay, drop-target math via `calculateTargetIndex`, edge-zone auto-scroll) plus `LivePreviewReorderDecorator` (neighbour-shift live preview), exactly as `StepsSection` does for step cards. Each section is wrapped in a live-preview wrapper; the `slotProvider` derives slots from the wrapper components (struts are ignored). On drop, the container computes `(from, to)` and reorders the run via `TestRunSupport.moveCase(run, from, to)`, emitting the whole updated run through `onRunChange` and rebuilding the sections. The drop-indicator line is painted in `paintChildren` and suppressed while live-preview is active (the opening slot is the indicator). Drag-to-reorder is active only in the multi-case view (two or more sections); with fewer sections no handle gesture is attached. The `DragReorderSupport` instance is created lazily once the enclosing `JBScrollPane` is resolved in `addNotify`, so auto-scroll targets the real run-editor scroll pane; until then sections render without DnD.

## 15g. Create Test Run dialog (multi-case)

`CreateTestRunDialog` is the entry point for building a `.tr.md` run from one or more existing test cases. The triggering `CreateTestRunAction` collects every `.tc.md` leaf under `test-cases/` as a `CandidateCase` (key, title, and `CaseFacets`: status, priority, tags, environments), then opens the dialog. The dialog produces a `CreateTestRunRequest` (selected files, destination, file name, import options, title) consumed by `CreateMultiCaseRunWriter`.

**Default run file name.** The default name is `Run-<timestamp>.tr.md`, where the timestamp is `yyyy-MM-dd HH:mm` with both the date/time space and the `:` separator replaced by `-` (e.g. `Run-2026-06-27-04-29.tr.md`). The default name must never contain spaces or colons so it is a legal single filename on every platform without further escaping.

**Adaptive filter section.** A filter row is shown only for facets that actually vary or exist across the candidate cases, computed by `CreateRunFacets.present(...)`: status and priority appear only when the candidates hold two or more distinct values; tags and environments appear only when at least one candidate has any. When no facet qualifies, the whole filter section is omitted. Each present facet is a `ComboBox` laid out in a three-column grid with no per-combo text labels and no titled "Filter" separator; the placeholder text alone identifies each control.

**Placeholder dropdowns.** Each filter combo starts unselected (`selectedIndex = -1`) and renders its facet name as muted placeholder text (`UIUtil.getContextHelpForeground`) via `placeholderRenderer` only while no value is chosen and only in the closed button state; the placeholder is never added as a selectable list entry. Selecting a value applies it to `CreateRunFilter`; the live case checklist, footer count, auto-title, and OK-button validation refresh on every change.

**Cases checklist.** Below the filter, a scrollable checklist shows the cases matching the current filter, each a checkbox toggling membership. All cases start unchecked, so the user opts into each case explicitly (or via "Select all"). It is a native `com.intellij.ui.CheckBoxList` (a `JBList`) hosted in a `ScrollPaneFactory.createScrollPane(...)` so it never shows a horizontal scrollbar and a case title wider than the list is clipped rather than producing horizontal scroll. A clipped title is revealed in full by the platform's expandable-item hover popup (the same affordance the tool-window TCs/TRs trees use), not by a Swing tooltip, so the reveal behavior is consistent with the TCs tab; using the platform scroll-pane factory (rather than a bare `JBScrollPane`) keeps the vertical scrollbar from painting over that hover popup. When no case matches the filter, the list is replaced by an empty-state label. The header carries compact native "Select all" / "Clear" buttons (custom `JButton` subclasses that hug their text, because macOS Darcula/Aqua enforce a wide minimum button width that ignores `setMargin()`), which check/uncheck all currently-visible cases. A footer label shows the selected count. OK is enabled only when at least one case is selected and the destination and file name are valid.

**Import options.** Below the destination/file-name fields, an Import section carries the same `RunImportOptions` checkboxes as the single-case dialog: `Description`, `Tags`, `Environment`, `Tickets`, `Links`, `Attachments`. In the multi-case dialog all six options are **disabled (unchecked) by default** - a multi-case run starts as a clean result sheet, and the user opts into copying any source content. `Description` gates whether each selected case's body blocks are copied into its run section; `Tags` and `Environment` are additionally interactable only when at least one candidate carries that facet (otherwise they stay disabled with the empty-source tooltip). The chosen options apply uniformly to every selected case when `CreateMultiCaseRunWriter` builds the run.

**Accessibility.** Every labeled field caption (Title, Destination, File name) is linked to its field via `labelFor`, and the "Cases" caption is linked to the checklist, so screen readers announce a name for each. The four filter combo boxes carry an explicit `accessibleName` (their placeholder label) because the placeholder lives only in the cell renderer and is otherwise invisible to assistive tech. Inline validation errors are non-focusable labels, so each error is also mirrored into the offending field's `accessibleDescription` (cleared when valid), letting a screen-reader user hear the problem on focus. (Disabled import checkboxes still carry their "nothing to import" reason only in a tooltip, which a screen reader cannot reach while the control is disabled; this is an accepted platform limitation.)

**Layout integrity.** `createCenterPanel` lays the dialog out in a single `GridBagLayout` via `fullRow`/`labeledRow` helpers, each advancing a shared row counter. Because an unqualified `gridy` inside a `GridBagConstraints().apply { ... }` block resolves to the constraints' own field rather than the enclosing local, both helpers capture the row index in a local before entering `apply`; otherwise full-width rows (filter, cases list, footer, the Import separator, and the import panel) collapse onto a single grid row and overlap.
