...
- UI layer: **pure Swing + JBUI for custom editor surfaces, Kotlin UI DSL v2 for dialogs and settings. No Compose / Jewel in `src/`.** `TestCasePanel` (at `editor/ui/TestCasePanel.kt`) and `TestRunPanel` (at `run/TestRunPanel.kt`) are `JPanel` subclasses mounted by `SpeqaPreviewEditor` / `TestRunEditor` inside a `JBScrollPane`. Shared inline Markdown rendering is owned by `editor/ui/primitives/MarkdownReadOnlyPane` (a `JEditorPane` wrapper fed by `org.intellij.markdown` CommonMark parser output); SpeQA does not own any custom inline-Markdown parser. `build.gradle.kts` does not reference `org.jetbrains.kotlin.plugin.compose` or `composeUI()`; `src/` contains no `androidx.compose.*`, `org.jetbrains.jewel.*`, `@Composable`, or `JewelComposePanel` references.
- **Swing engineering rules:**
    ...
    - **Single-line rows for links, attachments, and tickets.** `LinkRow`, `AttachmentRow`, and the ticket-chip containers in the test-case header and in each step must never wrap their contents to a second visual line regardless of the preview width. Titles/filenames truncate with the native `JLabel` ellipsis; trailing action icons (edit, delete, remove-chip) stay pinned at the row's right edge. The implementation pattern for `LinkRow` and `AttachmentRow`: root `BorderLayout` with `WEST` = leading icon (fixed), `CENTER` = title/name `JBLabel` with `minimumSize = Dimension(0, prefHeight)` so `BorderLayout` can shrink it below its preferred width and let the JLabel paint "...", `EAST` = actions panel (`FlowLayout`-wrapped `SpeqaIconButton`s). `FlowLayout` at the row root is forbidden because it wraps overflowing children to a second line that gets clipped by the fixed row height. Each row also pins `minimumSize`/`maximumSize` to `preferredSize.height` so the enclosing `BoxLayout.Y_AXIS` in `LinkList`/`AttachmentList` does not stretch or squash individual rows into overlapping bounds.
    ...
    - **Uniform icon brightness for all inline action icons.** Every secondary-action icon across Speqa renders at the same muted alpha, exported as the `SPEQA_MUTED_ICON_ALPHA = 0.6f` constant in `SpeqaIconButton.kt`. Both `speqaIconButton` and `mutedActionLabel` apply this constant via `IconLoader.getTransparentIcon`. `speqaIconButton`'s `muted` parameter defaults to `true` so callers do not need to opt in; the result is that edit pencils (links, attachments, title, ID), delete/trash and chip-close icons, section-header `+` buttons (tags, environment, links, attachments), `+` buttons inside `TagCloud`/`TicketRow`, the `mutedActionLabel` leading icons on "Add ticket ID" / "Add link" / "Attach file", and the hover-revealed drag-handle icon on step cards (tinted via `Label.disabledForeground`) all share a single visual weight. There is no longer a separate "primary" vs "muted" icon brightness for inline actions: the only distinction is that the save checkmark in edit mode also uses the same muted alpha so it does not appear brighter than the pencil that opened edit mode.
...
- Section headers and grouped labels use SpeQA's Swing primitives and IntelliJ theme colors; no Jewel component dependency remains in the product code or plugin manifest.
...
- **Theme-switch background:** `SpeqaPreviewEditor` and `TestRunEditor` subscribe to `LafManagerListener` and `EditorColorsListener` and re-read the editor canvas background via `EditorColorsManager.getInstance().globalScheme.defaultBackground`, reassigning it to both the root `JPanel` and the mounted `TestCasePanel` / `TestRunPanel`. The mounted panels additionally call `SwingUtilities.updateComponentTreeUI(this)` on theme events.
...
- **ID duplicate detection:** `SpeqaIdRegistry.isDuplicate()` returns true when count > 1. Annotator underlines `id:` in frontmatter and offers an `Assign next free ID` quick fix that replaces the duplicate id value in frontmatter with `IdSet.nextFreeId()` for the matching id type. Editor header shows red text + tooltip. VFS listener triggers rescan on Speqa file changes; 2s periodic refresh in all editors catches cross-file duplicates. Does not block saving
...
After the Compose → Swing migration (Step 6), two popups are restored natively via `JBPopupFactory`:

- **TagCloud autocomplete popup** (`editor/ui/chips/TagAutocompletePopup.kt`): anchored below (or above when vertical room is insufficient) the `TagCloud` add input. Contents are all known project tags from `SpeqaTagRegistry` minus `currentTags`, filtered by case-insensitive substring of the input text. The input keeps focus; `Down`/`Up` move list selection, `Enter` picks the highlighted item and clears the input, `Esc` closes the popup. Empty filter result hides the popup temporarily (no empty list visible). A single popup instance is reused across keystrokes — only the list model is refreshed. Popup width matches anchor width with a `180 px` minimum. The popup is non-modal and requests no focus so the input's `DocumentListener` keeps firing.
- **Metadata picker popup** (`editor/ui/chips/MetadataPickerPopup.kt`): anchored below a component (chip row / link / ticket input), lists matching `.tc.md` / `.tr.md` files resolved via `SpeqaTagRegistry`-adjacent indexes (or a caller-supplied `List<VirtualFile>`). Rows are rendered with a custom `ListCellRenderer` that shows optional ID text, the file title, and a truncated relative path via the pure `indexedFileMatchFrom` / `IndexedFileMatchDisplay` helpers in `MetadataActions.kt`. Keyboard `Enter` or mouse click invokes `onPick(VirtualFile)` and closes the popup. Empty results render a muted "no matches" label using bundle key `metadata.noMatches`. The current file is marked with `metadata.current` rather than being filtered out (preserves the Compose-era contract).
- **Bundle keys involved:** `tagCloud.addTag`, `tagCloud.inputPlaceholder`, `metadata.noMatches`, `metadata.current`, plus the existing `metadata.popupTitle.*` family for titles when used.
...
`TagChip` exposes two optional callbacks: `onClick: (() -> Unit)?` and `contextMenu: (() -> JPopupMenu)?`. Left-click invokes `onClick`; right-click builds and shows `contextMenu()` at the press location. Keyboard activation (`Enter`/`Space`) is unchanged and still bound to `onActivate`. `TagCloud` accepts a `MetadataScope` parameter (`TEST_CASES` or `TEST_RUNS`) and, when a `Project` is available, wires chips to:
...
The Swing `TestCasePanel` and `TestRunPanel` render their content in a single vertical `BoxLayout.Y_AXIS` viewport with page padding of 12 px. The composition matches the Compose-era reference and is shared between the case and run surfaces (with run-specific substitutions noted below).
