...
- `TestCasePreview` holds `focusRequestStepIndex` state and passes it + setter to `StepsSection`.
...
- **Attachment data model:** `Attachment(val path: String)` represents a single file attachment referenced by its path relative to the Markdown file that contains it. There is no project-root-relative mode and no legacy fallback path interpretation. `TestStep` carries `actionAttachments` and `expectedAttachments` (both `List<Attachment>`, default empty). `TestCase` carries a top-level `attachments: List<Attachment>` field (declared after `tags`, before `bodyBlocks`, default empty) for case-level attachments. All attachment lists default to empty so existing files without attachments parse without changes.
...
- **Attachment parsing — per-step:** Within the scenario section, attachment lines (matching the same two formats) that appear after a step's action lines and before the next step or blockquote are collected as `actionAttachments` for that step. Attachment lines that appear after an expected-result blockquote (`>`) line are collected as `expectedAttachments` for that step. Attachment lines are never appended to the action text. Both attachment lists are flushed onto the current step when a new step starts or at end of the scenario section.
...
- **Attachment UI — StepCard integration:** `StepCard` accepts optional `actionAttachments: List<Attachment>`, `expectedAttachments: List<Attachment>`, `onActionAttachmentsChange: (List<Attachment>) -> Unit`, `onExpectedAttachmentsChange: (List<Attachment>) -> Unit`, `project: Project?`, `tcFile: VirtualFile?`, and `onOpenFile: (Attachment) -> Unit` parameters (all with safe defaults). An `AttachmentList` is rendered after the action field's Row (inside the outer Column) and after the expected field's Row (inside the `if (expected != null)` block, after the expected Row). Attachment lists are only rendered when both `project` and `tcFile` are non-null (guarded by `if` check).
- **Attachment UI — StepsSection integration:** `StepsSection` accepts optional `project: Project?` and `tcFile: VirtualFile?` parameters (after `onTestCaseChange`), forwarded to each `StepCard`. Attachment change callbacks update the step's `actionAttachments` / `expectedAttachments` via `testCase.copy(steps = testCase.steps.updated(index, step.copy(...)))`. The `onOpenFile` callback resolves the attachment strictly relative to the current markdown file via `AttachmentSupport.resolveFile` and opens it with `FileEditorManager`. `TestCasePreview` passes its `project` and `file` parameters to `StepsSection` as `project` and `tcFile`.
...
- **Attachment test coverage:** `TestCaseParserTest` verifies: (1) parsing a general `Attachments:` section with bare bracket syntax yields `testCase.attachments`; (2) step action attachments (bare bracket lines between action and `>`) yield `step.actionAttachments`; (3) step expected attachments (bare bracket lines after `>`) yield `step.expectedAttachments`; (4) standard Markdown link/image syntax (`[text](path)` and `![alt](path)`) in the `Attachments:` section is parsed correctly. `TestCaseSerializerTest` adds `import io.github.barsia.speqa.model.Attachment` and verifies a round-trip (`round trip preserves attachments`): a `TestCase` with case-level attachments, `actionAttachments`, and `expectedAttachments` serializes and re-parses with all attachment paths and counts preserved. The round-trip test constructs a `TestCase` directly using named constructor parameters for `actionAttachments` and `expectedAttachments` on `TestStep`.
- **Tag/environment normalization contract:** `tags` and `environment` follow the same frontmatter normalization rules everywhere SpeQA indexes or filters them. A YAML list becomes multiple values. A quoted scalar containing commas becomes one value. An unquoted comma-separated scalar becomes multiple values. An unquoted scalar without commas becomes one value. Filtering and lookup always operate on these normalized values, never on raw frontmatter text.
...
- **Interactive chip behavior in test runs:** In `TestRunPanel`, only tag chips are interactive. Left-click filters SpeQA test runs by the clicked tag. Hover uses a short sentence-case tooltip (`Show runs with this tag`). Right-click menu contains `Find Test Runs With This Tag`, `Find Test Cases With This Tag`, and `Copy Tag`. The run `Environment` stays as the original editable text field only; SpeQA does not render a second chip-based environment section in test runs.
...
- Attachment lines (`![...](...)`, `[...](...)`-style image/link, or bare `[path]`) inside step blocks are recognized by the parser and routed into `actionAttachments` / `expectedAttachments` instead of accumulating into action, expected, or comment text. Pattern: `STEP_ATTACHMENT_PATTERN = Regex("""^!?\[.*]\(.*\)$|^\[.*]$""")`.
...
**Tags in test runs:**
- `TestRun` carries `tags: List<String> = emptyList()` copied from the source test case at run creation time. Tags are read-only in the test run (they snapshot the tags at run start).
- `TestRunSupport.createInitialRun` copies `testCase.tags.orEmpty()` into the resulting `TestRun.tags`. The `tags` field is placed after `title` in the `TestRun` constructor call inside `createInitialRun`.
- `TestRunSerializer` writes `tags:` in YAML list format (same as test case frontmatter) when the list is non-empty; the line is omitted when empty. The tags block is written after `runner:` and before the closing `---`. The verdict line is now written as `   — ${step.verdict.label}` (no `**bold**` markers). The `appendStepResult` private helper is updated accordingly. Serializer only writes `started_at` and `finished_at` if non-null. Steps are written directly after frontmatter (no `## Step Results` heading). No `## Summary` section is written. Multiline `step.expected` is written as multiple `>` lines via `step.expected.lines().forEach { line -> appendLine("   > $line") }`.
- `TestRunParser` parses `tags` from frontmatter using `SpeqaMarkdown.parseStringList`. `VERDICT_PATTERN` accepts only the current plain format (`— passed`, `— failed`, `— skipped`, `— blocked`). `SCENARIO_MARKER` is the only step-section marker; `Steps:` is not accepted. Step comments are parsed only from explicit `Comment:` lines; plain indented text is never treated as a comment. `parseStepResults` parses the entire body, stopping at any `## ` heading or EOF. Multiline expected: all `>` lines before the verdict are accumulated with `\n` joins as `expected`, not just the first line. The `summary` field is removed from `TestRun` data class and from the parser — no `summary = ...` in the `TestRun(...)` constructor call. Parser `startedAt` defaults to null when field is absent (not `LocalDateTime.now()`). `result` defaults to `NOT_STARTED` when field is absent. `LocalDateTime` import retained for `parseDateTime` helper. `TestRunSupport.deriveRunResult` returns `RunResult` (non-nullable) with `NOT_STARTED` for empty/all-NONE/all-SKIPPED cases. `createInitialRun` sets `startedAt = null` and no longer takes `startedAt` parameter. `SpeqaEditorSupport.startTestRun` no longer passes `startedAt` to `createInitialRun`. The local variable is renamed from `startedAt` to `now` since it's only used for file naming. The `createInitialRun` call no longer passes `startedAt`. `TestRunPanel` removes `onNext`, `currentStepIndex` parameters and the actions row. `StepResultRow` removes `isCurrent` parameter and the blue accent bar. `TestRunPanel` adds `createdLabel: String` parameter for file creation date. Header shows Created, Started (if non-null), Finished (if non-null). Result display: when `result == RunResult.NOT_STARTED`, the `ListComboBox` and "Result:" label are hidden; only a muted "Not started" text is shown. When `result != RunResult.NOT_STARTED`, the "Result:" label and `ListComboBox` (Passed/Failed/Blocked) are shown instead. Bundle key `run.notStarted` added. Bundle key `run.tooltip.created` added for created date label. Header date icons: `calendarCreated.svg` for Created date, `calendarUpdated.svg` for Started date, `calendarUpdated.svg` for Finished date. `TestRunEditor` tracks `startedAt` and `finishedAt` as mutable state, auto-sets startedAt on first interaction, auto-sets finishedAt when all steps have verdicts, clears finishedAt when all verdicts are NONE. Passes `createdLabel` to panel resolved from git/file creation date. Removes `currentStepIndex` state and `onNext` callback. The `summary` mutable state and `onSummaryChange` callback are removed from `TestRunEditor`; `summary` is removed from `saveToDocument`. Fix: `suppressDocumentRefresh` is set before `invokeLater` and reset inside `finally` after write completes. Tests updated: parser/serializer fixtures use `Scenario:` and `Comment:` only, with no legacy syntax. Serializer tests updated: `startedAt` is now nullable, tests pass explicit `LocalDateTime` when needed. Tests verify no `## Step Results` heading and no `## Summary` section. `summary`-related test cases removed. `TestRunSupport.currentStepIndex` method removed (no longer needed without Next navigation). `TestRunTest` updated: default result is `NOT_STARTED`, unknown `fromString` returns `NOT_STARTED`. Both test assertions updated; `summary` default assertion removed. `SpeqaIconProvider` handles `NOT_STARTED` by showing the default test run icon. Default fallback changed from `RunResult.FAILED` to `RunResult.NOT_STARTED`.
...
- Test coverage follows the same strictness: `TestRunParserTest` and `TestRunSerializerTest` use only `Scenario:` and `Comment:` fixtures, and they must not retain assertions for legacy markers or legacy verdict syntax.
- `TestRunPanel` accepts `tags: List<String> = emptyList()` as a parameter (positioned after `title`, before `runId`). When non-empty, tags are displayed in the header after the title `Text` and before the progress `Row`, as a `FlowRow` of `TagChip` (from `TagCloud.kt`, visibility `internal`) rendered directly — not wrapped in `TagCloud`. This avoids `TagCloud`'s Column wrapper which adds its own `tightGap` spacing, creating uneven vertical gaps inside the header's `compactGap`-spaced Column. Each chip uses colored styling (`colored = true`) with `onRemove = null` (read-only).
- `TestRunEditor` passes `initialRun.tags` to `TestRunPanel` as the `tags` parameter. `TestRunEditor.saveToDocument` wraps the `CommandProcessor.executeCommand` + `runWriteAction` call inside `ApplicationManager.getApplication().invokeLater({ ... }, ModalityState.defaultModalityState())` to ensure the write always happens on the EDT. The `TestRun` constructed in `saveToDocument` includes `tags = initialRun.tags`. The imports `ApplicationManager` and `ModalityState` are added to `TestRunEditor.kt`. The `saveToDocument` method captures the current values of `runId`, `manualResult`, `overriddenResult`, `environment`, `runner`, `stepResults`, and `isFinished` before scheduling, so the snapshot is taken at call time rather than at invokeLater execution time. The `summary` snapshot variable is removed from `saveToDocument`; the `TestRun(...)` constructor call in `saveToDocument` no longer has a `summary =` argument.
- `TestRunEditor` has a `DocumentListener` that re-parses the document on external changes (same pattern as `SpeqaPreviewEditor`). A `refreshTimer` (300ms debounce, `isRepeats = false`) re-parses via `TestRunParser.parse(document.text)` and updates all mutable state (`runId`, `stepResults`, `currentStepIndex`, `environment`, `runner`, `summary`, `isFinished`, `manualResult`, `overriddenResult`, `title`, `tags`). A `suppressDocumentRefresh` flag prevents re-entrant updates when `saveToDocument` writes to the document — set to `true` before the `invokeLater` call and reset to `false` inside the `finally` block after the write completes. The `title` and `tags` fields are promoted from `initialRun` constants to mutable state so the listener can update them. The `refreshTimer` is stopped in `dispose()`. The `TagChip` composable in `TagCloud.kt` is `internal` (not `private`) to allow direct use from `TestRunPanel`. The document listener is registered in `init {}` with `this` as the disposable parent so it auto-unregisters on editor disposal. The `javax.swing.Timer` import is added for the refresh timer. The compose panel reads from the mutable `title` and `tags` state instead of `initialRun.title`/`initialRun.tags`. The `saveToDocument` uses `suppressDocumentRefresh` and the `TestRun` constructed uses the mutable `title` and `tags`. Init registers the document listener; dispose stops the refresh timer and the ID state.
...
- `StepResult` has fields: `action`, `expected`, `verdict`, `comment`, `actionAttachments: List<Attachment> = emptyList()`, `expectedAttachments: List<Attachment> = emptyList()`. Default verdict is `NONE`. The `actionAttachments` and `expectedAttachments` carry step-level attachments copied from the test case at run creation.
- `TestRun` has fields: `id`, `title`, `tags`, `priority: Priority? = null`, `status: Status? = null`, `manualResult`, `startedAt`, `finishedAt`, `result`, `environment`, `runner`, `bodyBlocks: List<TestCaseBodyBlock> = emptyList()`, `links: List<Link> = emptyList()`, `attachments: List<Attachment> = emptyList()`, `comment: String = ""`, `stepResults`. The `priority` and `status` fields are copied from the test case at run creation. The `bodyBlocks`, `links`, and `attachments` fields carry the test case's body content, links, and attachments. The `comment` field stores an overall run comment (text after the last step in the file body).
...
- `TestRunSupport.createInitialRun` copies `priority`, `bodyBlocks`, `links`, `attachments` from the test case (no longer copies `status`). Step results copy `actionAttachments` and `expectedAttachments` from each test step. Sets `startedAt = null` (not started yet). No longer passes `startedAt`, `finishedAt`, or `result` explicitly — they use defaults. Unused imports removed, duplicate import fixed in `TestRunSupport.kt`.
...
- Test run files carry the full content from the test case: body blocks (description, preconditions), links, attachments, and step attachments — making runs fully self-contained.
- Serializer writes body blocks after frontmatter (before steps), then links section (`Links:\n\n[title](url)\n`), then attachments section (`Attachments:\n\n` with image/file format), then `Scenario:` marker followed by numbered steps, then overall comment if non-blank.
...
- **Step attachment parsing in test run:** attachment lines inside step blocks are parsed and collected into `actionAttachments` or `expectedAttachments` (depending on whether they appear before or after expected blockquote lines). The parser uses `parseAttachmentLine` to extract `Attachment(path)` from each matched line. Attachment lists are flushed onto the current step when a new step starts or at end of section, matching the same flush pattern as `TestCaseParser.parseSteps`.
...
- Each `StepResultRow` wraps its content in a `Row` with `height(IntrinsicSize.Min)`. The first child is a `Box(Modifier.width(2.dp).fillMaxHeight().background(barColor))` — a colored vertical bar. Bar color by verdict: `NONE` → `Color.Transparent`; `PASSED` → `SpeqaThemeColors.commitFlash`; `FAILED` → `SpeqaThemeColors.destructive`; `SKIPPED` → `SpeqaThemeColors.mutedForeground.copy(alpha = 0.3f)`; `BLOCKED` → `SpeqaThemeColors.accent`. Row uses `horizontalArrangement = Arrangement.spacedBy(SpeqaLayout.compactGap)`.
- The expected text in `StepResultRow` is preceded by a label (`label.expected` bundle key = "Expected") rendered at 11sp SemiBold in `mutedForeground` color, matching the style used in test case `StepCard`. The label and text are wrapped in a `Column` with `Arrangement.spacedBy(SpeqaLayout.itemGap)` vertical spacing.
...
- **Step workspace:** each step is a full-width row with a fixed number gutter and the action text field directly next to the number — no "Action" label between the number and the field. Expected result below. Both action and expected fields are multiline (`singleLine = false`, `minHeight = 48` in `PlainTextInput`) — they grow vertically as the user types. The step list must occupy the majority of the canvas height and feel like the main work area
...
- `tags: List<String>` — copied from test case at creation (readonly in run)
...
- `environment: String`
- `runner: String`
- `bodyBlocks: List<TestCaseBodyBlock>` — description + preconditions, copied from test case (readonly in run)
- `links: List<Link>` — copied from test case at creation (readonly in run)
- `attachments: List<Attachment>` — test case-level attachments, copied (readonly in run)
...
- `actionAttachments: List<Attachment>` — copied from test case step (readonly in run)
- `expectedAttachments: List<Attachment>` — copied from test case step (readonly in run)
...
**TestRunEditor — wiring `comment` state (Task 6):**
- `TestRunEditor` tracks `comment` as `private var comment by mutableStateOf(initialRun.comment)`.
- `refreshTimer` block updates `comment = parsed.comment` when document changes externally.
- `saveToDocument` captures `val snapshotComment = comment` and passes `comment = snapshotComment` to `TestRun(...)`. Also passes `priority`, `bodyBlocks`, `links`, `attachments` from `initialRun` (no `status`). Fields are round-tripped without loss. Test `parses priority and status` updated to `parses priority` (status assertion and `Status` import removed).
- `TestRunPanel` call site passes `priority`, `bodyBlocks`, `links`, `attachments` from `initialRun`, plus `comment = comment` and `onCommentChange` lambda that sets `comment = it`, calls `maybeSetStartedAt()`, and `saveToDocument()`.
...
- **Header structure:** The header `Column` (with `compactGap` spacing and `headerSurface` background) contains: ID + dates row, Title, Tags (if non-empty), Progress + Result row, then a `SurfaceDivider()`, then the Environment/Runner `Row`. This matches the test-case header pattern where metadata is separated from the title area by a divider. The Environment/Runner section is NOT a separate section in the root column — it lives inside the header column to maintain consistent `compactGap` spacing. The wrapping `Column(modifier = Modifier.padding(horizontal = SpeqaLayout.contentInset))` that previously surrounded Environment/Runner as a standalone section is removed — the header already provides `contentInset` padding.
- After the header (which includes Environment/Runner), `bodyBlocks` are rendered readonly with `blockGap` (`12dp`) between the header and the first body block, matching the test-case preview: `DescriptionBlock` with non-blank markdown shows a `SectionHeaderWithDivider` ("Description") + `Text`; `PreconditionsBlock` with non-blank markdown shows a `SectionHeaderWithDivider` ("Preconditions") + `Text`.
- After body blocks, attachments and links are laid out side by side in a `Row` (same as `TestCasePreview`): two `Column`s each with `Modifier.weight(1f)`, wrapped in `Row` with `contentInset` horizontal padding. If only attachments exist, show just attachments full width. If only links, show just links full width. If both, side by side. Each uses the shared `AttachmentRow` (with `onDelete = null`) and `LinkRow` (with `onEdit = null, onDelete = null`). `TestRunPanel` accepts `onOpenAttachment: (Attachment) -> Unit` parameter. `TestRunEditor` wires it to resolve the attachment via `AttachmentSupport.resolveFile` and open in `FileEditorManager`.
...
- The links readonly section in `TestRunPanel` uses the shared `LinkRow` composable with `onEdit = null` and `onDelete = null` (both defaulting to null per signature) instead of inline `Row` + icon + text rendering. Clicking opens the URL in the system browser via `BrowserUtil.browse`. `TestRunPanel` imports `AttachmentRow` and `LinkRow` from `io.github.barsia.speqa.editor.ui`. The private `ReadonlyAttachmentRow` composable at the bottom of `TestRunPanel.kt` is deleted; all its call sites are replaced with `AttachmentRow`. `AttachmentSupport` import is removed from `TestRunPanel.kt` since it is no longer referenced there after `ReadonlyAttachmentRow` is deleted.
...
**Header:** FlowRow(16dp)[progress, Row[result+dropdown], tags]. Passed uses `passedIndicator`. TestRunPanel root: `.background(SpeqaThemeColors.surface)` for theme switch.
