...
**Import options.** Below the destination/file-name fields, an Import section carries the same `RunImportOptions` checkboxes as the single-case dialog: `Description`, `Tags`, `Environment`, `Tickets`, `Links`, `Attachments`. `Description` (default enabled) gates whether each selected case's body blocks are copied into its run section; `Tags` and `Environment` are enabled only when at least one candidate carries that facet; `Tickets`, `Links`, and `Attachments` default to disabled. The chosen options apply uniformly to every selected case when `CreateMultiCaseRunWriter` builds the run.
