...
**State persistence:** the tree's expansion state and selection are persisted per project (in the workspace file) and restored when the project is reopened and across IDE restarts, using the platform `TreeState` mechanism (the same one the Project view uses) held by a project-level `PersistentStateComponent`. The current state is captured when the tool-window content is disposed (which includes project close) and re-applied when the content is built. Nodes are identified by their backing file, so expansion/selection survive the tree rebuilds caused by VFS changes and filtering. The in-memory filter selection itself is not persisted (see "Filters").
