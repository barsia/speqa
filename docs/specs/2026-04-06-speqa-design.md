...
- **Run title editing:** the test-run title uses the same inline-editing mechanics as the test-case title. `InlineEditableTitleRow` is reused: pencil to enter edit mode, `Enter` to commit, `Escape` to cancel, blur to commit, and focus return to the title affordance after save/cancel. The run title additionally commits **live** on every keystroke (`liveCommit = true`), matching the runner field, so the frontmatter `title` updates as the user types rather than only on commit; the case-mode title keeps commit-on-blur. While editing, the pencil shows a save (disk) icon and `Save` tooltip; clicking it commits.
