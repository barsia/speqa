...
A `.tr.md` run that contains two or more cases renders through `RunMultiCasePanel` instead of the flat `TestCasePanel(mode = RUN)`. The layout is chosen once from the initial parse via `RunCasesContainer.layoutFor(run)` (`SECTIONED` when `run.cases.size >= 2`, else `FLAT`). The panel shows a run header (editable title, runner, overall-result pill, "N / M cases done" progress) above a `RunCasesContainer`: a vertical `BoxLayout.Y_AXIS` list of one collapsible `RunCaseSection` per case, separated by a scaled 16 px vertical strut. Every edit folds the changed case back into the whole `TestRun` and emits it through `onRunChange` (full reserialize; there is no per-case patch path).
