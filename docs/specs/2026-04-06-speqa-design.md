...
- **Scroll synchronization:** In split mode, the text editor (left) and visual editor (right) keep their scroll positions synchronized proportionally. The synchronization is bidirectional:
  ...
  - **Editor → preview:** `VisibleAreaListener` computes the left editor fraction and writes the right `JBScrollPane.verticalScrollBar.value` on EDT. It only mirrors events where the editor viewport's vertical offset changed; document edits can emit visible-area events because content height or wrapping changed, but those reflow-only events must not move the preview away from the edited text.
  - **Preview → editor:** the preview scroll bar `AdjustmentListener` computes the right panel fraction and calls `editor.scrollingModel.scrollVertically()` on EDT via `invokeLater`
