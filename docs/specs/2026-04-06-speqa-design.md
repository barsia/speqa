...
**Run header field order and actions.** Both the single-case and multi-case run headers order the operational fields as `Result | Progress` then `Runner | Priority`, where the multi-case run leaves the Priority slot empty (no run-level priority across multiple cases). The Progress value uses the default label foreground in both (no muted tint). Each run header has a right-aligned action row of icon buttons: a **Reset results** button (both runs) that, after a confirmation dialog, clears every result via `TestRunSupport.resetResults` (all step verdicts to NONE, every case result and the run result to NOT_STARTED, manual overrides dropped, start/finish timestamps cleared; step text, comments, and metadata preserved); the multi-case run additionally has **Expand all** / **Collapse all** buttons that call `RunCasesContainer.setAllExpanded(true/false)` to expand or collapse every case section at once.
