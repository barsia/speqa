...
**Overall run result.** The run-header result is the same interactive pill component as the per-case pills, not a read-only label. By default it shows the computed aggregate of the per-case results (`TestRunSupport.aggregateResult`); picking a result from its chooser sets a manual override on the whole run (`TestRunSupport.overrideRunResult`, which sets `TestRun.result` and `manualResult = true`) that holds regardless of the individual case results and does NOT modify any per-case result or pill. Picking "Auto (from cases)" clears the override (`TestRunSupport.clearRunOverride`) and returns to the computed aggregate. While the run result is manually overridden, a muted manual indicator appears next to the pill. The effective displayed value is `effectiveRunResult(run)` = the override when `manualResult` is set, otherwise the aggregate; when not overridden, the stored `TestRun.result` is kept in sync with the aggregate as cases change. The override persists across reserialize via the existing run-level `result:` / `manual_result:` front-matter fields.

**Per-case result override.** Each section header carries an interactive result pill (the shared `ResultPill` component) showing the case's `RunResult`. The pill is a clickable control (hand cursor, native tooltip) that opens an action-group popup listing the meaningful run results (Passed / Failed / Blocked / In progress / Not started) plus an "Auto (from steps)" entry. Picking a result calls `TestRunSupport.overrideCaseResult(case, picked)` (sets the result and `manualResult = true`); picking "Auto (from steps)" calls `TestRunSupport.clearCaseOverride(case)` (re-derives the result from the step verdicts and clears the manual flag). The chosen case is emitted through `onCaseChange`, which the container folds back into the whole run. When a case result is manually overridden (`RunCaseSectionState.isManual`), a small muted manual indicator (icon + label, theme-derived foreground) appears next to the pill; it is hidden when the result is derived from steps. `update(newCase)` refreshes the pill result and the manual-indicator visibility.
