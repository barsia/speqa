...
internal class AttachmentRow(
    ...
    init {
        ...
        if (!readOnly) {
            val slot = RemovableRowActionSlot(
                tooltip = SpeqaBundle.message("tooltip.removeAttachment"),
                onAction = onDelete,
            )
            removeSlot = slot
            // Suppress the attachment hover preview while the cursor is over the
            // remove action so the native button tooltip can surface instead.
            slot.actionComponent.addMouseListener(object : MouseAdapter() {
        ...
        // Skip icon tinting for missing attachments (the error-red colour
        // must stay) and for system file icons (multi-colour, not a
        // tintable SVG). Just hover-tint the filename text.
        if (!isMissing) {
            installRowHover(this, nameLabel, iconLabel = null, tintableIcon = null)
        }
