...
import io.github.barsia.speqa.editor.ui.primitives.emptyRowPlaceholder
...
internal class LinkList(
    ...
    private fun rebuild() {
        removeAll()
        rows.clear()
        if (links.isEmpty() && showEmptyPlaceholder) {
            add(emptyRowPlaceholder(SpeqaBundle.message("label.noLinks")))
        ...
        if (!hideAddButton) {
            addButton.alignmentX = LEFT_ALIGNMENT
            add(addButton)
        }
        revalidate()
        repaint()
