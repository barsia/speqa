...
class StepMetaRow(
    ...
    private fun buildLinkBlock(): JComponent {
        ...
        val runMode = mode == StepMetaMode.RUN
        return LinkList(project, hideAddButton = runMode, readOnly = runMode) { onLinksChange(it) }
    }

    private fun buildAttachmentBlock(): JComponent {
        if (project == null || tcFile == null) return emptyColumn()
        val runMode = mode == StepMetaMode.RUN
        return AttachmentList(project, tcFile, hideAddButton = runMode, readOnly = runMode) { onAttachmentsChange(it) }
    }
    ...
    private fun openTicketInBrowser(ticket: String) {
        val proj = project ?: return
        val url = io.github.barsia.speqa.settings.SpeqaSettings.getInstance(proj).resolveTicketUrl(ticket)
