        ...
        val panel = object : JPanel(java.awt.FlowLayout(java.awt.FlowLayout.LEFT, JBUI.scale(4), 0)) {
            override fun paintComponent(g: Graphics) {
                ...
                if (addStepFocused) {
                    ...
                    val hpad = JBUI.scale(4)
                    val vpad = JBUI.scale(2)
                    val rx = (label.x - hpad).coerceAtLeast(0)
                    val ry = (label.y - vpad).coerceAtLeast(0)
                    val rw = (label.width + hpad * 2).coerceAtMost(width - rx)
                    val rh = (label.height + vpad * 2).coerceAtMost(height - ry)
                    if (rw > 0 && rh > 0) {
