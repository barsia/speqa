...
class LivePreviewReorderMathTest {
    ...
    @Test
    fun `dragged card is hidden in its source slot while drag is active`() {
        assertEquals(
            false,
            livePreviewShouldPaintCard(
                cardIndex = 2,
                draggedIndex = 2,
                phase = LivePreviewPhase.DRAGGING,
            )
        )
    }

    @Test
    fun `non dragged cards stay visible during live preview`() {
        assertEquals(
            true,
            livePreviewShouldPaintCard(
                cardIndex = 1,
    ...
    @Test
    fun `dragged card stays hidden while ghost is returning on cancel`() {
        assertEquals(
            false,
            livePreviewShouldPaintCard(
                cardIndex = 2,
